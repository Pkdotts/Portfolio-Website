...
import {
  ...
  deleteSkill,
  updateSkill,
} from "@/app/api/controllers/skillController";
import {
  createExperience,
  deleteExperience,
  updateExperience,
} from "@/app/api/controllers/experienceController";
import {
  createEducation,
  deleteEducation,
  updateEducation,
} from "@/app/api/controllers/educationController";
import {
  createHobby,
  deleteHobby,
  updateHobby,
} from "@/app/api/controllers/hobbyController";
import TableRowButtons from "@/components/ui/dashboard/tablerowbuttons";
import { IconPlus } from "@tabler/icons-react";
import {
  DashboardModal,
  ModalActions,
} from "@/components/ui/dashboard/dashboardmodal";
        ...
        <TableTbody key={t.name_en}>
            ...
            <TableTd align="right">
              <Tooltip label="Add skill">
                <ActionIcon
                  size="sm"
                  variant="subtle"
                  onClick={() => openCreateModal(t.name_en)}
                >
                  <IconPlus />
                </ActionIcon>
              </Tooltip>
          ...
          {t.Skill.map((s) => (
            <TableTr key={s.skillId}>
              ...
              <TableRowButtons
                id={s.skillId}
                openEditModal={() => openEditModal({ type: "skill", data: s })}
                openDeleteModal={() =>
      ...
      <TableTbody>
        {experience.map((e) => (
          <TableTr id={e.workId.toString()} key={e.workId}>
            <TableTd>{e.position_en}</TableTd>
            <TableTd>{e.position_fr}</TableTd>
            <TableTd>{e.company}</TableTd>
            <TableTd>{e.description_en}</TableTd>
            <TableTd>{e.description_fr}</TableTd>
            <TableTd>{e.startDate}</TableTd>
            <TableTd>{e.endDate}</TableTd>
            <TableTd />
            <TableRowButtons
              id={e.workId}
              openEditModal={() =>
                openEditModal({ type: "experience", data: e })
              }
              openDeleteModal={() =>
                openDeleteModal({ type: "experience", data: e })
              }
            />
          </TableTr>
        ))}
      ...
      <TableTbody>
        {education.map((e) => (
          <TableTr id={e.educationId.toString()} key={e.educationId}>
            <TableTd>{e.diploma}</TableTd>
            <TableTd>{e.school}</TableTd>
            <TableTd>{e.startYear}</TableTd>
            <TableTd>{e.endYear}</TableTd>
            <TableTd />
            <TableRowButtons
              id={e.educationId}
              openEditModal={() =>
                openEditModal({ type: "education", data: e })
              }
              openDeleteModal={() =>
                openDeleteModal({ type: "education", data: e })
              }
            />
          </TableTr>
        ))}
      ...
      <TableTbody>
        {hobbies.map((h) => (
          <TableTr id={h.hobbyId.toString()} key={h.hobbyId}>
            <TableTd>{h.name_en}</TableTd>
            <TableTd>{h.name_fr}</TableTd>
            <TableTd />
            <TableRowButtons
              id={h.hobbyId}
              openEditModal={() => openEditModal({ type: "hobby", data: h })}
              openDeleteModal={() =>
                openDeleteModal({ type: "hobby", data: h })
              }
            />
          </TableTr>
        ))}
...
export default function AboutDashboard({
  ...
  const skillItem = selectedItem?.type === "skill" ? selectedItem.data : null;

  const experienceItem =
    selectedItem?.type === "experience" ? selectedItem.data : null;

  const educationItem =
    selectedItem?.type === "education" ? selectedItem.data : null;

  const hobbyItem = selectedItem?.type === "hobby" ? selectedItem.data : null;
    ...
    <>
      <Container w="100%" p="xs" size="md">
        <Paper bdrs="md">
          ...
          <Divider />
          <Tabs defaultValue="skills" value={activeTab} onChange={setActiveTab}>
            <TabsList justify="center" grow>
              <TabsTab value="skills">Skills</TabsTab>
              <TabsTab value="hobbies">Hobbies</TabsTab>
      ...
      {activeTab === "experience" ? (
        <DashboardModal<WorkExperience>
          ...
          item={experienceItem}
          itemName={
            experienceItem?.position_en + " - " + experienceItem?.company
          }
        ...
        >
          <div>
            ...
            <Group grow>
              <Input
                name="positionEn"
                placeholder="Your position in english"
                defaultValue={experienceItem?.position_en}
                required
              />
              <Input
                name="positionFr"
                placeholder="Your position in French"
                defaultValue={experienceItem?.position_fr ?? undefined}
              />
            </Group>
          </div>
          <div>
            ...
            <Input
              name="company"
              placeholder="The place you worked at"
          ...
          <div>
            ...
            <Group grow>
              <Textarea
                name="descriptionEn"
                placeholder="A brief synopsis in english"
                defaultValue={experienceItem?.description_en}
                required
              />
              <Textarea
                name="descriptionFr"
                placeholder="A brief synopsis in french"
                defaultValue={experienceItem?.description_fr ?? undefined}
              />
            </Group>
          </div>
          <Group grow>
            ...
            <div>
              ...
              <NumberInput
                name="endDate"
                placeholder="(Leave EMPTY if same as start year)"
                defaultValue={experienceItem?.endDate ?? undefined}
              />
            </div>
          </Group>
        </DashboardModal>
        ...
        >
          <div>
            ...
            <Input
              name="diploma"
              placeholder="Your diploma, please"
              defaultValue={educationItem?.diploma}
              required
            />
          </div>
          <div>
          ...
          <Group grow>
            <div>
              ...
              <NumberInput
                ...
                defaultValue={educationItem?.startYear}
                required
              />
            </div>
            <div>
              ...
              <NumberInput
                name="endYear"
                placeholder="(Leave EMPTY if same as start year)"
                defaultValue={educationItem?.endYear ?? undefined}
              />
            </div>
          </Group>
        </DashboardModal>
        ...
        >
          <div>
            ...
            <Input
              name="name_en"
              placeholder="Your hobby name, please"
              defaultValue={hobbyItem?.name_en ?? ""}
              required
            />
          </div>
          <div>
            ...
            <Input
              name="name_fr"
              placeholder="(Leave EMPTY if same as english)"
              defaultValue={hobbyItem?.name_fr ?? ""}
            />
          </div>
        </DashboardModal>
      ) : (
        activeTab === "skills" && (
          <DashboardModal
            ...
            createAction={(formData) =>
              createSkill(
                formData,
                selectedSkillType ?? skillItem?.skillType ?? "Others",
              )
            }
          ...
          >
            <div>
              ...
              <Input
                name="name_en"
                placeholder="Your skill name, please"
                defaultValue={skillItem?.name_en}
                required
              />
            </div>
            <div>
              ...
              <Input
                name="name_fr"
                placeholder="(Leave EMPTY if same as english)"
                defaultValue={skillItem?.name_fr ?? undefined}
              />
            </div>
          </DashboardModal>
        )
