...
async function SkillsPaper({
  skillTypes,
}: {
  skillTypes: SkillTypeWithSkills[];
}) {
                ...
                <div key={pickLocalizedName(t.name_en, t.name_fr)}>
                  {t.Skill.length > 0 && (
                    <Stack gap="0">
                      ...
                      <Group m="xs">
                        {t.Skill.map((s) => (
                          <Badge key={s.skillId} variant="outline" size="lg">
                            {s.name_en}
                          </Badge>
                        ))}
                      </Group>
...
async function ExperiencePaper({
  experience,
}: {
            ...
            {experience.map((e) => (
              <InnerPaper key={e.workId}>
                <Group justify="space-between">
                  <Title order={4}>
                    {pickLocalizedName(e.position_en, e.position_fr)}
                  </Title>
                ...
                <Text size="sm">
                  {pickLocalizedName(e.description_en, e.description_fr)}
                </Text>
              </InnerPaper>
            ))}
          </Stack>
          ...
          <Stack>
            {education.map((e) => (
              <InnerPaper key={e.educationId}>
                ...
                <Group gap="xs" justify="space-between"></Group>
